...
#	    return;
#	}
        if ($apply_stop && (my $price = $order->is_executed($calc, $i)) )
        {
            $self->apply_order($order, $price, $calc->prices->at($i)->[$DATE]);
            return;
        }
    }
...
sub stats {
    ...
    foreach (@{$self->{'details'}})
    {
        ...
        if ($_->is_buy_order) {
            ...
            $vobose = $vobose . " buy at " . $_->{'date'} .  " in " . sprintf("%.2f", $_->price);
        } elsif ($_->is_sell_order) {
            $ugain += $_->price;
            $nb_sell_orders++;
            $sold += $sum;
            $quantity_sold += $quantity_factor * $_->quantity;
            $vobose = $vobose . " sell at " . $_->{'date'} .  " in " . sprintf("%.2f", $_->price);
        } elsif ($_->is_sell_order) {
        }
