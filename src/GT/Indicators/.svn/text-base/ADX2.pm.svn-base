...
# $Id: ADX.pm 664 2009-04-20 19:03:02Z ras $
...
sub initialize {
    ...
    $self->{'dm'} = GT::Indicators::DMI->new([ $self->{'args'}->get_arg_names(1)]);
...
sub calculate {
    ...
    $positive_di_value = $indic->get($self->{'dm'}->get_name(0), $i);
	$negative_di_value = $indic->get($self->{'dm'}->get_name(1), $i);
